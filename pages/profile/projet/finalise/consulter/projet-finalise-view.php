...
if ($_GET) {
    ...
    $query = "SELECT * FROM projet, promoteur, secteur, personne WHERE projet.id_promoteur_fk_projet = promoteur.id_promoteur AND projet.id_secteur_fk_projet = secteur.id_secteur AND promoteur.id_personne_fk_promoteur = personne.id_personne AND id_projet = $id_projet AND etat_projet = 'finalise'";

    $statement = $db->prepare($query);
    $statement->execute();
    $rowStatement = $statement->rowCount();
    if ($rowStatement <= 0) {
        header("Location:/realproject/pages/profile/projet/finalise");
        exit();
    }
    ...
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item active"><a href="/realproject/pages/profile/projet/finalise/">Projets finalisé</a></div>
            ...
            <div class="col-md-4">
                <div class="card"><img class="card-img-top" src="/realproject/pages/profile/assets/img/<?= si_funct($result['img_cover_projet'], '', 'image-vide/img1.png', 'img-projet/' . $result['img_cover_projet']) ?>" alt="Placeholder">
                    ...
                    <div class="card-body">
                        ...
                        <div class="" style="text-decoration: underline; cursor: pointer;" data-toggle="tooltip" data-placement="bottom" title="Contactez l'administrateur pour plus d'information">Ce projet a été finalisé le <?= date("d-m-Y", strtotime($result['date_finalisation_projet'])) ?> à <?= date("H:i:s", strtotime($result['date_finalisation_projet'])) ?> </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).on('submit', '#id_update_projet_form', function(event) {
        ...
        $.ajax({
            url: "projet-finalise-view-action.php",
            type: "POST",
