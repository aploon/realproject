...
$colonne = array("date_publication_projet", "date_fin_projet", "date_finalisation_projet", "nom_secteur", "nom_projet", "montant_collecte_projet", "montant_total_projet");
...
$query .= "SELECT * FROM projet, promoteur, secteur, personne WHERE projet.id_promoteur_fk_projet = promoteur.id_promoteur AND projet.id_secteur_fk_projet = secteur.id_secteur AND promoteur.id_personne_fk_promoteur = personne.id_personne AND etat_projet = 'finalise' AND id_promoteur = $id_promoteur "; // changer
...
function get_total_all_records($db)
{
	...
	$statement = $db->prepare("SELECT * FROM projet, promoteur, secteur, personne WHERE projet.id_promoteur_fk_projet = promoteur.id_promoteur AND projet.id_secteur_fk_projet = secteur.id_secteur AND promoteur.id_personne_fk_promoteur = personne.id_personne AND etat_projet = 'finalise' AND id_promoteur = $id_promoteur "); // same query as above
	$statement->execute();
